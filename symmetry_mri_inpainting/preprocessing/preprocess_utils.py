...
def symmetrize_image(image, mask, axis=2):
    ...
    for i in range(image.shape[axis]):
        ...
        image_center = int(np.nan_to_num(np.round(np.mean(np.nonzero(np.sum(image_slice, axis=0))))))
        mask_center = int(np.nan_to_num(np.round(np.mean(np.nonzero(np.sum(mask_slice, axis=0))))))
        ...
        symmetrized_image_slice = (
            image_slice * symmetry_mask + \
            image_slice[:,::-1] * np.logical_not(symmetry_mask)
        )
...
def process_images(input_folder, output_folder, seed=42):
    ...
    for subdir, _, files in os.walk(input_folder):
        ...
        for t1n_file in t1n_files:
            ...
            processed_healthy_mask_image_path = os.path.join(output_subdir, t1n_file.replace('t1n', 'mask'))
            processed_unhealthy_mask_image_path = os.path.join(output_subdir, t1n_file.replace('t1n', 'mask-unhealthy'))
            nib.save(nib.Nifti1Image(t1n_image, np.eye(4)), processed_t1n_image_path)
            nib.save(nib.Nifti1Image(healthy_mask_image, np.eye(4)), processed_healthy_mask_image_path)
            nib.save(nib.Nifti1Image(unhealthy_mask_image, np.eye(4)), processed_unhealthy_mask_image_path)
            ...
            healthy_voided_image_path = os.path.join(output_subdir, t1n_file.replace('t1n', 'healthy-voided'))
            unhealthy_voided_image_path = os.path.join(output_subdir, t1n_file.replace('t1n', 'unhealthy-voided'))
            nib.save(nib.Nifti1Image(t1n_healthy_voided_image, np.eye(4)), healthy_voided_image_path)
            nib.save(nib.Nifti1Image(t1n_unhealthy_voided_image, np.eye(4)), unhealthy_voided_image_path)
            ...
            symmetrized_t1n_image_path = os.path.join(output_subdir, t1n_file.replace('t1n', 'symm-t1n'))
            symmetrized_healthy_mask_image_path = os.path.join(output_subdir, t1n_file.replace('t1n', 'symm-healthy-mask'))
            nib.save(nib.Nifti1Image(symmetrized_t1n_image, np.eye(4)), symmetrized_t1n_image_path)
