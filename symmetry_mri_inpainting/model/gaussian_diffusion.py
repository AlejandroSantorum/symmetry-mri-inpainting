...
class GaussianDiffusion:
    ...
    def p_sample_loop_known(
        ...
        the t1-voided image and the segmentation mask, therefore its shape is (N, 2, H, W).
        Noise will be appended in the last channel (NOTE, maybe the noise needs to be
        added in the first channel! Possible bug?)
        """
        if device is None:
    ...
    def p_sample_loop_progressive(
        ...
        org_MRI = img[
            :, :-1, ...
        ]  # original brain MR image (t1-voided + segmentation mask)
